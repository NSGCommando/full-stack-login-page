# Note Taker Web App #
A simple app to allow users to save notes and view them later. It can be easily built in Docker via the `docker-compose.yml` provided.
...
- **Database**
  ...
  - Table Schemas:
    ...
    - user_id refences UserData[id] for each note's ownership
    - Fields:
      - `id`: integer, is Primary Key
      - `user_id`: integer, Foreign Key Referencing UserData[id]
      - `note`: string, stores the user note, cannot be null
      - `timestamp`: string, stores formatted UTC timestamp, generated by backend automatically on note creation
